...
import type { ToolMetadata, McpContent, ContentBlock } from "../config/types.ts";
import { getServerPrefix } from "../config/types.ts";
import { lazyConnect, updateServerMetadata, updateMetadataCache, getFailureAgeSeconds } from "../lifecycle/lazy-connect.ts";
import { abortable, throwIfAborted } from "../utils/abort.ts";
import { buildToolMetadata, getToolNames, findToolByName, formatSchema } from "../cache/tool-metadata.ts";
...
function getAuthRequiredMessage(
  state: GatewayState,
  serverName: string,
  defaultMessage = `Server "${serverName}" requires OAuth authentication. Run mcp({ action: "auth-start", server: "${serverName}" }) to get a browser URL.`,
): string {
  return formatAuthRequiredMessage(state.config, serverName, defaultMessage);
}

function getAuthFailedMessage(state: GatewayState, serverName: string, message: string): string {
...
export function executeUiMessages(_state: GatewayState): ProxyToolResult {
  return {
    content: [{ type: "text" as const, text: "UI messages not available in standalone mode." }],
    details: { sessions: 0 },
  };
}

export function executeStatus(state: GatewayState): ProxyToolResult {
...
export async function executeAuthStart(state: GatewayState, serverName: string): Promise<ProxyToolResult> {
  const definition = state.config.mcpServers[serverName];
  if (!definition) {
    return {
...
export async function executeAuthComplete(state: GatewayState, serverName: string, input: string): Promise<ProxyToolResult> {
...
export function executeDescribe(state: GatewayState, toolName: string): ProxyToolResult {
...
export function executeList(state: GatewayState, server: string): ProxyToolResult {
  ...
  if (toolNames.length === 0) {
    ...
    return {
      content: [{ type: "text" as const, text: `Server "${server}" is configured but not connected. Use mcp({ connect: "${server}" }) to retry.` }],
...
export async function executeConnect(state: GatewayState, serverName: string, signal?: AbortSignal): Promise<ProxyToolResult> {
