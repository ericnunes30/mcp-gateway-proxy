...
export async function getValidToken(
  ...
  if (expired === true && entry.tokens.refreshToken) {
    ...
    console.error(`MCP Auth: Token expired for ${serverName}, attempting refresh`)
    ...
    try {
      ...
      if (!clientInfo) {
        console.error(`MCP Auth: No client info for refresh for ${serverName}`)
        return null
      }
...
export async function removeAuth(serverName: string): Promise<void> {
  ...
  await clearOAuthState(serverName)
  console.error(`MCP Auth: Removed credentials for ${serverName}`)
}
