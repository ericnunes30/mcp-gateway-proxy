// tool-metadata.ts - Tool metadata building and schema formatting
import type { ToolMetadata, McpTool, McpResource, ServerEntry } from "../config/types.ts";
import { formatToolName, isToolExcluded } from "../config/types.ts";

// Generic state interface (replaces McpExtensionState)
interface ToolMetadataState {
...
export function buildToolMetadata(
  ...
  for (const tool of tools) {
    ...
    metadata.push({
      ...
      uiResourceUri: undefined, // Not extracted in standalone mode
    });
  }
...
export function getToolNames(state: ToolMetadataState, serverName: string): string[] {
  return state.toolMetadata.get(serverName)?.map(m => m.name) ?? [];
}

export function totalToolCount(state: ToolMetadataState): number {
  let count = 0;
